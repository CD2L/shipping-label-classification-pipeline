...
def fake_other():
    ...
    if rand <= 0.2:
        ...
        date_format = ['%Y-%m-%d', '%y-%m-%d', '%y-%b-%d', '%d/%m/%Y', '%d/%m/%y', '%d %b %y','%d %b %Y', '%d %B %y','%d %B %Y']
        rand_format = random.randint(0,len(date_format)-1)
        sentence = " ".join(faker.words(nb_word)) + " " + faker.date_this_century().strftime(date_format[rand_format])
    else:
