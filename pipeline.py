...
def yolo(url):
    ...
    model_label_detection.augment = 0.5
...
def addr_ident_test(
    ...
    if iter > len(df.label):
        raise Exception(
            f"Iter maximum for this file is {len(df.label)} ({iter} provided)"
        )
...
def ocr_test(
    test_dir="./utils/data-examples/ocr/", json_file="references.json", verbose=True
):
        ...
        for idx, file in enumerate(files):
            if file.name.endswith((".png", ".jpeg", ".jpg")):
                ...
                bleu_score = sentence_bleu(
                    references, sentence.split(), weights=(0.5, 0.3, 0.2, 0)
                )
